...
| F-025 | **HIGH** | Expansão de foothold via cred-stuffing — 2ª conta interna `test2@soultv.com.br:123456` (id=18, is_staff=false); admin@tv.com/marketing@tv.com NEGATIVOS (is_staff NÃO conquistado) | cms.soultv.com.br (signin) | evidence/F-025.txt | Confirmado (is_staff não atingido) |
| F-026 | **HIGH** | Stripe `getPaymentToken` (validação F-011): proxy unauth à Stripe LIVE confirmado — mapeamento de campos `number/month/year/code`→`card[*]`, HTTP 402 (raw-card APIs disabled na conta = minting atualmente mitigado); CORS `*`; card-only | tv-iteractiva Firebase CF | evidence/F-026.txt | Confirmado (minting mitigado por config Stripe) |
| F-027 | MEDIUM | Tokens de conteúdo PPV/account = base64(XOR-plaintext ASCII, chave fixa) — NÃO assinados/JWT; forjáveis em princípio (known-plaintext); impacto reduzido (F-018 já bypassa paywall) | cms.soultv.com.br (content_token/PPV token) | evidence/F-027.txt | Confirmado (análise estrutural) |
| F-028 | MEDIUM | SSRF tcommerce (F-023) — superfície re-confirmada (escrita de URL arbitrária sem auth/allowlist); OOB NEGATIVO (backend não fetcha sincrono; scraper async não disparou na janela); canário criado e removido | api-tcommerce.soultv.com.br | evidence/F-028.txt | Candidate (OOB negativo) |
| F-029 | INFO | Re-validação foothold + caça a emails staff em relatórios financeiros (negativa) + top-up cred-stuff admin@tv.com → 50 senhas = 0 hit (threshold atingido). is_staff NÃO alcançável no escopo read-only | cms.soultv.com.br / api-tcommerce | evidence/F-029.txt | Confirmado (is_staff objetivo encerrado) |
...
- ⏳ Acesso admin/staff (is_staff): não conquistado (mass assignment rejeitado em signup e em `POST /v1/account`; campos `is_staff`/`is_superuser`/`role`/`is_admin`/`is_premium`/`plan` ignorados pelo backend). **Cred-stuffing esgotado dentro do threshold** (admin@tv.com 50 senhas = 0 hit — F-029; relatórios financeiros F-015 contêm APENAS customers, sem emails staff — hipótese descartada). Is_staff NÃO alcançável no escopo read-only atual (requer wordlist maior, acesso a mailbox p/ reset oracle F-020, ou cred vazada via SSRF→metadata — não confirmado).
